...
_SCENE_BASE_QUALITY = (
    "single illustration frame, cinematic composition, consistent storybook style"
)
...
def _negative(negative_prompt: str) -> str:
    return (
        f"{_SCENE_NEGATIVE}, {negative_prompt}" if negative_prompt else _SCENE_NEGATIVE
    )
...
def _build_text(
    scene_prompt: str, visual_style: str, negative_prompt: str, seed: int
...
def _build_location(
    ...
    workflow, images = _build_redux(
        scene_prompt, visual_style, negative_prompt, location_b64, seed, "reference.png"
    )
