...
def build(portrait_prompt: str, negative_prompt: str = "", character_id: str | None = None) -> dict:
    ...
    workflow["33"]["inputs"]["text"] = (
        ...
        f"hands cut off, back view, profile view, side view, sitting pose, multiple characters"
        + (f", {negative_prompt}" if negative_prompt else "")
    )
    workflow["10"]["inputs"]["noise_seed"] = deterministic_seed(character_id) if character_id else random_seed()
    return workflow
